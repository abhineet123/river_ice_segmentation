...
class DenseNet:
    def __init__(self, height, width, ch, nclass, psi_act_type=0, ksiz=3, nfilters=16, loss_type=0):
        ...
        hidden1 = tf.layers.conv2d(self.X, filters=nfilters, kernel_size=(ksiz, ksiz), padding="same",
                                   activation=tf.nn.elu,
                                   kernel_initializer=he_init)
        hidden2 = tf.layers.conv2d(tf.concat([self.X, hidden1], axis=3), filters=nfilters, kernel_size=(ksiz, ksiz),
                                   padding="same",
                                   activation=tf.nn.elu, kernel_initializer=he_init)
        hidden3 = tf.layers.conv2d(tf.concat([self.X, hidden1, hidden2], axis=3), filters=nfilters,
                                   kernel_size=(ksiz, ksiz),
                                   padding="same", activation=tf.nn.elu, kernel_initializer=he_init)
        hidden4 = tf.layers.conv2d(tf.concat([self.X, hidden1, hidden2, hidden3], axis=3), filters=nfilters,
                                   kernel_size=(ksiz, ksiz), padding="same", activation=tf.nn.elu,
                                   kernel_initializer=he_init)
        hidden5 = tf.layers.conv2d(tf.concat([self.X, hidden1, hidden2, hidden3, hidden4], axis=3), filters=nfilters,
                                   kernel_size=(ksiz, ksiz), padding="same", activation=tf.nn.elu,
                                   kernel_initializer=he_init)
        hidden6 = tf.layers.conv2d(tf.concat([self.X, hidden1, hidden2, hidden3, hidden4, hidden5], axis=3),
                                   filters=nfilters,
                                   kernel_size=(ksiz, ksiz), padding="same", activation=tf.nn.elu,
                                   kernel_initializer=he_init)
        hidden7 = tf.layers.conv2d(tf.concat([self.X, hidden1, hidden2, hidden3, hidden4, hidden5, hidden6], axis=3),
                                   filters=nfilters, kernel_size=(ksiz, ksiz), padding="same", activation=tf.nn.elu,
                                   kernel_initializer=he_init)
        hidden8 = tf.layers.conv2d(
            tf.concat([self.X, hidden1, hidden2, hidden3, hidden4, hidden5, hidden6, hidden7], axis=3),
            filters=nfilters, kernel_size=(ksiz, ksiz), padding="same", activation=tf.nn.elu,
            kernel_initializer=he_init)
        hidden9 = tf.layers.conv2d(
            tf.concat([self.X, hidden1, hidden2, hidden3, hidden4, hidden5, hidden6, hidden7, hidden8], axis=3),
            filters=nfilters,
        ...
        psi = tf.reshape(tf.layers.conv2d(
            tf.concat([self.X, hidden1, hidden2, hidden3, hidden4, hidden5, hidden6, hidden7, hidden8, hidden9],
                      axis=3),
        ...
        if loss_type == 0:
            ...
            for i in range(nclass):
                class_idx = self.class_indices[i]
                gather_self = tf.gather_nd(diff, class_idx)
                self.loss_convnet += tf.nn.l2_loss(gather_self)
