...
% remove last row corresponding to conf 1.0 that can sometimes distort plots especially if plot_ext=0
remove_last_row = 0;
adjust_y = 0;
sort_x = 0;
...
% has to be tex for colored text in the legend
...
if rec_prec_mode
    ...
    for line_id = 1:n_lines
        ...
        line_legend = fscanf(fileID, '%s', 1);
        plot_legend{line_id} = line_legend;

        split_legend = split(line_legend, '--');

        if length(split_legend) == 2
            plot_legend(line_id) = split_legend(1);
            line_cols(line_id) = split_legend(2);
        end
        ...
        if enable_auc
            ...
            elseif enable_auc == 3
                ...
                % find the row where recall becomes maximum and copy
                % this row into any rows before it while making the precision 0 and leaving the recall be
                [max_x, max_x_ind] = max(auc_X);

                for i__ = 1:max_x_ind - 1
                    auc_X(i__) = max_x;
                    auc_Y(i__) = 0;
                end

                % find the row where the precision becomes maximum
                % and copy this row into any rows after it while making
                % the recall zero and leaving the precision be
                [max_y, max_y_ind] = max(auc_Y);

                for i__ = max_y_ind + 1:n_data
                    auc_Y(i__) = max_y;
                    auc_X(i__) = 0;
                end
    ...
    if only_auc
        fprintf('exiting due to only_auc')
        return
    end
    ...
    elseif size(k.textdata, 2) == size(k.data, 2) + 1
        ...
        if exist('valid_columns', 'var')
            ...
            if bar_plot
                ...
                n_tick_labels = numel(xtick_labels);
                for line_id = 2:n_tick_labels
...
if exist('contains', 'builtin')
    ...
    if metric_in_title && ~contains(plot_title, metric, 'IgnoreCase', true)
...
if bar_plot
    ...
    if bar_vals
        y1 = k.data;
        % old method that doesn't place the text over bars horizontally

        % for k1 = 1:size(y1,2)
        % 	% Note: ‘XOffset’ Is An Undocumented Feature, This Selects The ‘bar’ Centres
        % 	xdt = bar_plt(k1).XData
        % 	xofst = bar_plt(k1).XOffset
        % 	ctr = bsxfun(@plus, bar_plt(k1).XData, bar_plt(k1).XOffset')
        %     ydt = bar_plt(k1).YData;                                     % Individual Bar Heights
        % 	if white_text
        % 		text_col = 'w';
        % 	else
        % 		text_col = 'k';
        ...
        %     text(ctr, ydt, sprintfc('%.1f', ydt), 'HorizontalAlignment','center', 'VerticalAlignment','bottom',...
        % 	 'FontSize', bar_vals_font_size,...
        ...
        for idxModel = 1:nModel
            ...
            text(bar_xPos, y1(idxModel, :), num2str(y1(idxModel, :)', fmt), ...
                'FontSize', bar_vals_font_size, ...
                'FontName', 'Times New Roman', ...
                'Color', text_col, ...
                'FontWeight', 'bold', ...
...
for i = 1:n_lines
    ...
    col_id = mod(i - 1, length(line_cols)) + 1;
    style_id = mod(i - 1, length(line_styles)) + 1;

    line_col = line_cols{col_id};
    line_style = line_styles{style_id};

    if enable_markers && exist('markers', 'var')
        marker_id = mod(i - 1, length(markers)) + 1;
...
if colored_legend
    ...
    for line_id = 1:n_lines
        col_id = mod(line_id - 1, length(line_cols)) + 1;
        line_col_rgb = col_rgb{strcmp(col_names, line_cols{col_id})};
        final_legend{line_id} = strrep(final_legend{line_id}, '_', ' ');
        % final_legend{line_id} = strrep(final_legend{line_id}, '_', '-')
        % final_legend{line_id} = replace(final_legend{line_id}, '_', '\_');
...
h_legend = legend(final_legend, 'Interpreter', legend_interpreter);
...
if hide_legend_border
	if paper
		set(h_legend, 'EdgeColor', "w");
	else
		set(h_legend, 'EdgeColor', "k");
	end
end
...
elseif enable_x_label
    y_label = strtrim(y_label);
    %y_label=sprintf("%s (%%)", y_label);
else
	y_label = strcat(x_label, " (%)");
    "sprintf replaces space with tab for some really annoying reason and this causes malfunctioning when rendering in the PDF";
	% y_label = sprintf("%s (%%)", x_label);
end
